...
export const AnimatedThemeToggler: React.FC<AnimatedThemeTogglerProps> = ({
  ...
  const toggleTheme = useCallback(() => {
    ...
    // innerWidth/innerHeight: percentages must resolve
    ...
    const applyTheme = () => {
      ...
      if (isControlled) {
        ...
        try {
          localStorage.setItem("agenticmeet_theme", newTheme ? "dark" : "light");
  ...
  return (
    <button
      ...
      onClick={toggleTheme}
      className={cn(
        "w-8 h-8 rounded-xl bg-[var(--purple-subtle)] hover:bg-[var(--purple-bg)] text-[var(--purple-primary)] border border-[var(--border-soft)] flex items-center justify-center transition-all cursor-pointer active:scale-95 shadow-xs",
        className
      )}
